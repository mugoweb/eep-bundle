...
class EepUrlAliasCreateGlobalCommand extends Command
{
...
<info>Usage</info>
-----
eep:urlalias:createglobal 'eznode:43' '/my-destination-path' eng-GB

eep:urlalias:createglobal 'module:/content/search' '/my-destination-path' eng-GB

See: ezsystems/ezplatform-kernel/eZ/Publish/Core/Persistence/Legacy/Content/UrlAlias/Mapper.php for resource types

EOD;

        $this
            ->setName('eep:urlalias:createglobal')
            ->setAliases(array('eep:ua:createglobal'))
            ->setDescription('Creates a global URL alias')
    ...
    {
        $inputResource = $input->getArgument('resource');
        ...
        if (!$confirm)
        {
            $confirm = $io->confirm(
                sprintf(
                    'Are you sure you want to create global URL alias "%s" for resource "%s"?',
                    $inputPath,
                    $inputResource
                ),
                false
        ...
        if ($confirm)
        {
            ...
            try
            {
                $urlAlias = $this->urlAliasService->createGlobalUrlAlias
                (
                    $inputResource,
                    $inputPath,
                    $inputLanguageCode,
                    ...
                    case 'table':
                        {
                            ...
                            (
                                new TableCell
                                (
                                    "{$this->getName()} [$inputResource $inputPath $inputLanguageCode]",
